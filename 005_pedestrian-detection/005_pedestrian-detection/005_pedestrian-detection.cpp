...
int main(int argc, char** argv) {
	string bin = "E:/models/tf_models/pedestrian-detection-adas-0002/pedestrian-detection-adas-0002.bin";
	string xml = "E:/models/tf_models/pedestrian-detection-adas-0002/pedestrian-detection-adas-0002.xml";
	...
	// 开视频窗口
	VideoCapture capture("H:/OpenVINO_Learning/005_pedestrian-detection/pedestrian.mp4");
	...
	while (true) {
		...
		if (OK == infer_request_curr->Wait(IInferRequest::WaitMode::RESULT_READY)) {
			...
			for (int row = 0; row < rows; row++) {
				...
				if (confidence > 0.5) {
					...
					putText(curr_frame, "pedestrain detection", Point(20, 20), FONT_HERSHEY_SIMPLEX, 0.75, Scalar(0, 0, 255), 2, 8);
		...
		imshow("ssd-model optimizer demo", curr_frame);
		char c = waitKey(2);
		if (c == 27) {
			break;
		}
